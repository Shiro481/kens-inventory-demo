-- =====================================================
-- FULL DEPLOYMENT SCRIPT FOR KEN'S INVENTORY SYSTEM
-- =====================================================
-- Generated based on current project state.
-- Includes: Schema, Views, Functions, Security, and Seed Data.
...
CREATE TABLE IF NOT EXISTS product_bulb_variants (
    ...
    color_temperature numeric(10,2), -- Updated to numeric to support range/sorting
    variant_color text, -- e.g., "Red", "6000K" (Display string)
    variant_sku text,
    ...
    -- Constraint: Depends on logic. If variant_id is null, unique might be on product_id + bulb_type + color?
    -- Creating a more flexible uniqueness index instead of strict constraint if needed.
...
SELECT 
    ...
    pbv.id as id, -- Unique ID for POS
    pbv.id as variant_id, -- The ID of the product_bulb_variant row
    COALESCE(btv.display_name, pbv.bulb_type) as display_name, -- Fallback to stored bulb_type
...
-- Public Access Policies (For Demo Scope)
...
-- Allow Insert/Update/Delete for authenticated users or public (depending on strictness)
-- For this demo, allowing public write to facilitate easy testing without login
...
-- Allow login lookup (public read for email check? No, usually handled by auth system, but for custom auth:)
CREATE POLICY "Allow Self Read" ON admins FOR SELECT USING (true); -- Simplified for login check
...
-- Bulb Type Variants (The Menu)
INSERT INTO bulb_type_variants (base_name, variant_name, compatibility_list, display_name, description) VALUES
('LED Headlight Kit', 'H4/H7/9005/9006', ARRAY['H4', 'H7', '9005', '9006'], 'Universal Kit (H4/H7/9005/9006)', 'Universal fit'),
...
('LED Signal', '1156/BA15S', ARRAY['1156', 'BA15S'], 'Single Contact (1156)', 'Turn signal'),
('Interior LED', 'T10/194/168', ARRAY['T10', '194', '168'], 'Wedge T10', 'Interior/Plate')
...
INSERT INTO admins (email, password_hash, role, full_name, is_active)
VALUES 
  (
    'shaqleeambagan101@gmail.com', 
    crypt('@Shaq481', gen_salt('bf')), 
    'super_admin', 
    'System Owner', 
    true
  )
