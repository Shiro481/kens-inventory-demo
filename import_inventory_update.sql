...
BEGIN
    ...
    -- We use ILIKE to find matches, assuming standard naming conventions in bulb_type_variants
    ...
    IF r6_product_id IS NULL THEN
        INSERT INTO products (
            ...
            headlight_cat_id,
            6000.00, 4500.00, 39, -- Sum of stock: 9+1+6+2+4+2+2+2+2+3+1 = 34? Wait, let's recount. 
            -- R6 Counts: 9+1+6+2+4+2+2+2+2+3+1 = 34. (Matches original R6 file)
    ...
    -- Helper function logic inline mostly or simple inserts.
    -- We use conflicts to do nothing if variant exists, assuming unique constraint on (product_id, variant_sku) or similar. 
    -- If no unique constraint, we check exists.
    ...
    IF rs7_product_id IS NULL THEN
        INSERT INTO products (
            ...
            headlight_cat_id,
            8500.00, 6000.00, 4, -- Sum: 1+1+1+1 = 4
            'GPNE RS7 Series.', 
            true,
            '{"warranty": "1 Year"}'::jsonb, -- Assumption on warranty
            '6000' -- All variants are 6000
    ...
    IF r3_product_id IS NULL THEN
        INSERT INTO products (
            ...
            'GPNE-R3', 'GPNE R3 LED Headlight', 'GPNE', -- Assuming brand
            headlight_cat_id,
            5000.00, 0.00, 2, -- Cost not provided, defaulting to 0
            'GPNE R3 Series.', 
            true,
            '{}'::jsonb
    ...
    -- R3 H1 (Qty: 2, No Color specified)
