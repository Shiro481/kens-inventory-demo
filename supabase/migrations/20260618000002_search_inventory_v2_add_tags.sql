-- Fix 4: Add p_tags parameter to search_inventory_v2 so chip-based tag filters
-- are applied server-side and work correctly across all paginated pages.
-- Also adds total_count (window function) so the store can display accurate
-- "X matching items" counts without a separate COUNT query.
